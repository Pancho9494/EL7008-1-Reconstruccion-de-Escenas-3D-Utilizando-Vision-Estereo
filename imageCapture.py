...
def capture(width, height, center):
    ...
    while left.isOpened():
        ...
        # Press s key to save picture
            ...
            print("Saving images")
            cv2.imwrite(f"images/left/{numCapture}.png", imgL)
            cv2.imwrite(f"images/right/{numCapture}.png", imgR)
            numCapture += 1
        # Press d key to empty folders
        elif key == ord('d'):
            print("Clearing folders")
            for file in glob.glob(f'images/left/*'):
                os.remove(file)
            for file in glob.glob(f'images/right/*'):
                os.remove(file)
